...
```bash
mw-docker-dev browsertest <extension> [spec]
```

For example:

```bash
mw-docker-dev browsertest TwoColConflict
mw-docker-dev browsertest TwoColConflict collapse.js
```

Prerequisites:
- install the extension's npm dependencies, see [here](#NPM install with Docker)

Watch testing live using VNC:
- host: `localhost:5900`
- password: `secret`

Notice:
- the TLD ".test" was needed for Chrome to correctly resolve the DNS
